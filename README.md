...
Data Grid: Populated with the current list of users fetched from the backend.
...
Form clears and title updates to "New User".

User fills out mandatory fields (Username, Display Name, Email, Role).

User clicks "Save User".

System: Validates input. If successful, saves to backend, displays a success toast/message, adds the new user to the Data Grid, and highlights the newly created row.

4.2 Editing an Existing User
User clicks a row in the Data Grid (e.g., ID 1, AdminUser).

The row becomes highlighted.

The form populates with AdminUser's data.

User modifies a field (e.g., changes Phone number).

"Save User" button becomes active.

User clicks "Save User".

System: Validates input, saves changes to backend, displays a success message, and refreshes the Data Grid to reflect updated values.
